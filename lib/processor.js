...
exports = module.exports = function Processor(command) {
  ...
  this.writeToStream = function(stream, callback, writableCallback) {
    if (!this.options._isStreamable) {
      callback(null, new Error('selected output format is not streamable'));
    } else {

      writableCallback = writableCallback || function(){return true;};
      ...
      this._prepare(function(err, meta) {
        if (err) {
          ...
          //handle timeout if set
          var processTimer, processKiller = function() {
            ffmpegProc.removeAllListeners('exit');
            ffmpegProc.kill('SIGKILL');
            callback(self.E_PROCESSTIMEOUT, 'timeout');
          };
          if (self.options.timeout) {
            processTimer = setTimeout(processKiller, self.options.timeout);
          }
          ...
          ffmpegProc.stdout.on('data', function(chunk) {
            stream.write(chunk);
            if ( writableCallback() === false ){
              processKiller();
              //callback(self.E_PROCESSTIMEOUT, 'Stopped');
            }
          });
          ...
          ffmpegProc.on('exit', function(code, signal) {
            ...
            if(/http/.exec(self.options.inputfile)) {
              ...
              var cb_ = function() {
                if (self.options.inputstream && self.options.inputstream.fd) {
                  fs.close(self.options.inputstream.fd, function() {
                    callback(code, stderr);
                  });
                } else if ( self.options.inputstream && self.options.inputstream.close ){
                  self.options.inputstream.close();
                } else if ( self.options.inputstream && self.options.inputstream.end ){
                  self.options.inputstream.end();
                } else {
                  callback(code, "Input stream will not be closed");
                }
              };
              if ( !stream.fd ) {
                stream.end ? stream.end() : callback(code, "Output stream will not be closed");
                cb_();
              } else {
